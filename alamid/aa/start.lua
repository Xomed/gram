...
api_id=11558238,
api_hash='f83aec5d0d59ea91073a0635ecde934b',
session_name=bot_id,
token=Token
}
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
namebot = redis:get(bot_id..":namebot") or "السلطان"
SudosS = {5088821183,1908202112,1198482178}
Sudos = {sudoid,5088821183,1908202112,1198482178}
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
function Bot(msg)  
local idbot = false  
...
JoinChannel = true
local url , res = https.request('https://api.telegram.org/bot5007930304:AAHctSzJyBAeYnQqGD7ttYvNAK9s4b4gC_Y/getchatmember?chat_id=@uui9u&user_id='..msg.sender.user_id)
...
type = 'inline',
data = {
{{text = '◉ ᴀʟᴤᴜʟᴛᴀɴ sᴏᴜʀᴄᴇ .',url="t.me/UUi9U"}},
}
}
...
reply_dev = bot.replyMarkup{
type = 'inline',data = {
	{{text = '◉ تحديث البوت .',data="UpBot"},{text = '◉ تحديث السورس الرسمي الرسمي.',data="UpSu"}},
{{text = '◉ قناه التحديثات .',url="t.me/UUi9U"}},
{{text = '◉ رجوع .',data="Can"}},
}
}
bot.editMessageText(chat_id,msg_id,"*◉︙قائمه التحديثات .*", 'md', true, false, reply_dev)
end --
if devS(data.sender_user_id) then
if Text == "Updates" then
reply_dev = bot.replyMarkup{
type = 'inline',data = {
	{{text = '◉ تحديث الرسمي .',data="UpSuUnVipDevB"},{text = '◉ تحديث الخارق .',data="UpSuVipDevB"}},
{{text = '◉ تحديث البوت .',data="UpBot"}},
{{text = '◉ رجوع .',data="Can"}},
}
}
bot.editMessageText(chat_id,msg_id,"*◉︙قائمه التحديثات .*", 'md', true, false, reply_dev)
...
{{text = '◉ تغيير اسم البوت .',data="namebot"},{text =(redis:get(bot_id..":namebot") or "السلطان"),data="delnamebot"}},
...
if Text == "UpSu" then
bot.answerCallbackQuery(data.id, "◉ تم تحديث السورس الرسمي", true)
os.execute('rm -rf start.lua')
os.execute('curl -s https://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@raw.githubusercontent.com/ALSULTANplus/u/main/libs/start.lua -o start.lua')
dofile('start.lua')  
end
if Text == "UpSuVipDevB" then
bot.answerCallbackQuery(data.id, "◉ تم تحديث السورس الخارق", true)
os.execute('rm -rf start.lua')
os.execute('curl -s https://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@raw.githubusercontent.com/ALSULTANplus/u/main/start.lua -o start.lua')
dofile('start.lua')  
end
if Text == "UpSuUnVipDevB" then
bot.answerCallbackQuery(data.id, "◉ تم تحديث السورس الرسمي", true)
os.execute('rm -rf start.lua')
os.execute('curl -s https://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@raw.githubusercontent.com/ALSULTANplus/u/main/libs/start.lua -o start.lua')
dofile('start.lua')  
...
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '⌁︙ sᴏᴜʀᴄᴇ ᴀʟsᴜʟᴛᴀɴ', url = 't.me/uui9u'}, },}} 
...
if programmer(msg) then  
bot.sendText(msg.chat_id,msg.id,"*◉︙تم تحديث السورس الرسمي الى الاصدار الجديد .*","md",true)
os.execute('rm -rf start.lua')
os.execute('curl -s https://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@raw.githubusercontent.com/ALSULTANplus/u/main/libs/start.lua -o start.lua')
dofile('start.lua')  
end
end
if text == ("تحديث الخارق") and devS(msg.sender.user_id) then 
bot.sendText(msg.chat_id,msg.id,"*◉︙تم تحديث السورس الخارق الى الاصدار الجديد .*","md",true)
os.execute('rm -rf start.lua')
os.execute('curl -s https://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@raw.githubusercontent.com/ALSULTANplus/u/main/start.lua -o start.lua')
dofile('start.lua')  
end
...
https.request("https://api.telegram.org/bot"..Token.."/sendphoto?chat_id=" .. msg.chat_id .. "&photo=https://t.me/UUi9U&caption=".. URL.escape(Text).."&photo=0&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
if text == "الساعه" then
bot.sendText(msg.chat_id,msg.id,"*◉︙الساعه الان : ( "..os.date("%I:%M%p").." ) .*","md",true)  
end
if text == "شسمك" or text == "سنو اسمك" then
namet = {"حجي اسمي "..(redis:get(bot_id..":namebot") or "السلطان"),"يابه اسمي "..(redis:get(bot_id..":namebot") or "السلطان"),"اني لقميل "..(redis:get(bot_id..":namebot") or "السلطان"),(redis:get(bot_id..":namebot") or "السلطان").." اني"}
bot.sendText(msg.chat_id,msg.id,"*"..namet[math.random(#namet)].."*","md",true)  
end 
if text == "بوت" or text == (redis:get(bot_id..":namebot") or "السلطان") then
...
local UserId_Info = bot.searchPublicChat("SsSsvS")
if UserId_Info.id then
...
mrd = redis:get(bot_id..":"..msg.chat_id..":marriage:"..msg.sender.user_id) 
if mrd then
if text == 'زوجي' then
lj = "*زوجج هو*"
...
Remsg = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
local UserInfo1 = bot.getUser(msg.sender.user_id)
local UserInfo = bot.getUser(Remsg.sender.user_id)
...
if redis:get(bot_id..":"..msg.chat_id..":marriage:"..Remsg.sender.user_id) then
return bot.sendText(msg.chat_id,msg.id,"*◉︙عزيزي/تي* ‹ "..us.." › \n*◉︙الشخص متزوج سابقا .*","md",true)
end
if redis:get(bot_id..":"..msg.chat_id..":marriage:"..msg.sender.user_id) then
return bot.sendText(msg.chat_id,msg.id,"*◉︙طلق الاولى بعدين اتزوج ثانيه .*","md",true)
end
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '‹ قبول ›', data = "marriage_"..msg.sender.user_id.."_"..UserInfo.id.."_"..msg.id.."_OK"},{text = '‹ رفض ›', data ="marriage_"..msg.sender.user_id.."_"..UserInfo.id.."_"..msg.id.."_No"}}}}
...
reply_to_message_id=msg.id,
reply_markup=markup(nil,{{{text = '◉ ᴀʟᴤᴜʟᴛᴀɴ sᴏᴜʀᴄᴇ .',url="t.me/UUi9U"}}})
})
end
end
if not redis:get(bot_id..":"..msg.chat_id..":settings:singforme") then
...
return send("sendphoto",{
chat_id=msg.chat_id,
photo="https://t.me/UUi9U",
caption=('*ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ sᴏᴜʀᴄᴇ ᴀʟᴤᴜʟᴛᴀɴ .\n\n*[◉  ᴀʟᴤᴜʟᴛᴀɴ ᴄʜᴀɴɴᴇʟ .](http://t.me/UUi9U)*\n\n*[◉  ɪɴғᴏ sᴏᴜʀᴄᴇ .](http://t.me/ui7uu)*\n\n*[◉  ᴀʟᴤᴜʟᴛᴀɴ ᴅᴇᴠᴇʟᴏᴘᴇʀ .](http://t.me/SsSsvS)*\n\n*[◉  ʙᴏᴛ ᴀʟᴤᴜʟᴛᴀɴ .](http://t.me/MRibot)'),
reply_to_message_id=msg.id,
parse_mode="markdown",
reply_markup=markup(nil,{{{text = '◉  ᴀʟᴤᴜʟᴛᴀɴ sᴏᴜʀᴄᴇ .',url="t.me/UUi9U"}}})
})
...
--else
---return bot.sendText(5088821183,0,data.luatele,"md",true)  
--
end
----------------------------------------------------------------------------------------------------
end
